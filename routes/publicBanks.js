...
router.get('/', jwtAuth, async (req, res) => {
    try {
        ...
        // Get user's company and entitlements
...
router.get('/for-survey', jwtAuth, async (req, res) => {
    try {
        const user = await User.findById(req.user.id).select('companyId subscription');
        ...
        allBanks.forEach(bank => {
            ...
            const bankData = {
                ...
                tags: bank.tags,
                questionCount: bank.questionCount,
                questions: Array(bank.questionCount).fill(null).map((_, i) => ({ _id: `placeholder_${i}` })), // Placeholder for question count
                lastUpdated: bank.updatedAt,
                type: bank.type === 'free' ? 'FREE' : 'PAID',
                price: bank.type === 'paid' ? bank.priceOneTime : undefined,
                accessType,
...
router.post('/:id/import', jwtAuth, async (req, res) => {
    try {
        ...
        // Check user's entitlement
        const user = await User.findById(req.user.id).select('companyId subscription email');
