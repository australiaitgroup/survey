		...
		<div className='space-y-4'>
			<div className='bg-white rounded-lg p-4 shadow-sm mb-4'>
				<div className='mb-2 text-xs sm:text-sm text-gray-500'>
					...
					<span className='text-gray-700'>
						{collection?.name || t('collections.title', 'Collections')}
					</span>
				</div>
				<div className='flex items-start justify-between gap-4'>
					...
					<div className='flex flex-wrap gap-2'>
						<button className='btn-outline' onClick={() => setShowModal(true)}>
							{i18n.t('buttons.edit', { ns: 'translation', defaultValue: 'Edit' })}
						</button>
						<button className='btn-outline' onClick={() => setShowSelectModal(true)}>
							{t('collections.addSurveys', 'Add Surveys')}
						</button>
					</div>
				</div>
			</div>
			...
			<div className='bg-white rounded-lg p-4 shadow-sm mb-4'>
				<h3 className='text-sm font-semibold text-gray-700 mb-3'>
					{t('collections.linkedSurveys', 'Linked Surveys')}
				</h3>
				{surveys.length === 0 ? (
					<div className='text-center py-8 text-gray-500'>
						{t('collections.noSurveys', 'No surveys found')}
					</div>
				) : (
									...
									<tr key={s._id} className='hover:bg-gray-50'>
										...
										<td className='px-4 py-2 whitespace-nowrap text-sm text-gray-700'>
											{(() => {
												...
												if (
													(s as any).sourceType === 'multi_question_bank'
												) {
													const list =
														(s as any).multiQuestionBankConfig || [];
													return list.reduce(
														(sum: number, cfg: any) =>
															sum + (cfg?.questionCount || 0),
														0
													);
												}
												if ((s as any).sourceType === 'manual_selection') {
													return ((s as any).selectedQuestions || [])
														.length;
												}
			...
			{showSelectModal && (
				<SelectSurveysModal
					...
					onSave={async ids => {
						await api.patch(`/collections/${collection._id}/surveys`, {
							surveyIds: ids,
						});
