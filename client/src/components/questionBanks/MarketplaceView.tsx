...
const MarketplaceView: React.FC = () => {
	...
	useEffect(() => {
		...
		if (paymentStatus === 'success' && bankId) {
			setSuccessMessage(
				t(
					'questionBanks.marketplace.paymentSuccess',
					'Payment successful! You now have access to this question bank.'
				)
			);
		...
		if (subscriptionStatus === 'success' && bankId) {
			setSuccessMessage(
				t(
					'questionBanks.marketplace.subscriptionSuccess',
					'Subscription started! You now have access to this question bank.'
				)
			);
			// Refresh data to update entitlements
			fetchPublicBanks({
		...
		if (paymentStatus === 'cancelled' || subscriptionStatus === 'cancelled') {
			setSuccessMessage(
				t('questionBanks.marketplace.paymentCancelled', 'Payment was cancelled.')
			);
					...
					<div className='flex-1 max-w-md'>
						<div className='relative'>
							<input
								...
								placeholder={t(
									'questionBanks.marketplace.searchPlaceholder',
							...
							<div className='absolute top-full mt-2 right-0 w-64 bg-white border border-gray-200 rounded-lg shadow-lg z-10 max-h-80 overflow-y-auto'>
								<div className='p-3'>
									<div className='text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2'>
										{t(
											'questionBanks.marketplace.availableTags',
											'Available Tags'
										)}
									</div>
				...
				<div className='bg-gray-50 rounded-lg p-8 text-center'>
					<div className='max-w-md mx-auto'>
						...
						<p className='text-gray-600'>
							{t(
								'questionBanks.marketplace.noResultsDescription',
					...
					{data.totalPages > 1 && (
						<div className='flex justify-center items-center gap-2 mt-6'>
							...
							<span className='ml-4 text-sm text-gray-600'>
								{t(
									'questionBanks.marketplace.pageInfo',
