...
interface UseLocalQuestionEditingParams {
  survey: Survey;
  setQuestionForms: (updater: (prev: Record<string, QuestionForm>) => Record<string, QuestionForm>) => void;
...
export const useLocalQuestionEditing = ({
  ...
  const addQuestionModalHandler = async (form: any) => {
    try {
      ...
      if (form.type !== QUESTION_TYPE.SHORT_TEXT && form.options) formData.options = form.options;
  ...
  const startEditQuestion = (surveyId: string, questionIndex: number) => {
    ...
    if (!questionType) {
      if (!question.options || question.options.length === 0) questionType = QUESTION_TYPE.SHORT_TEXT;
  ...
  const handleEditQuestionSubmit = async (form: QuestionForm) => {
    ...
    try {
      ...
      if (form.descriptionImage !== undefined) updateData.descriptionImage = form.descriptionImage;
  ...
  const removeQuestionEditOption = (
    ...
    if (currentForm) {
      setQuestionEditForms(prev => ({
        ...
        [formKey]: {
          ...currentForm,
          options: (currentForm.options || []).filter((_: any, i: number) => i !== optionIndex),
        },
      }));
    }
  };
    ...
    if (currentForm) {
      ...
      if (isCorrect) {
        if (Array.isArray(currentForm.correctAnswer)) {
          newCorrectAnswer = currentForm.correctAnswer.filter((i: number) => i !== optionIndex);
