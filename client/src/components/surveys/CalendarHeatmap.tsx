...
const CalendarHeatmap: React.FC<Props> = ({ data, weeks = 20, title, valueLabel }) => {
  ...
  const onEnter = (e: React.MouseEvent<HTMLDivElement>, key: string, v: number) => {
    ...
    setTooltip({ x: e.clientX + 12, y: e.clientY + 12, text: `${key}  ${label}`, visible: true });
  };
  const onMove = (e: React.MouseEvent<HTMLDivElement>) => setTooltip(prev => ({ ...prev, x: e.clientX + 12, y: e.clientY + 12 }));
  ...
  return (
    <div className='relative'>
      ...
      <div className='flex gap-1 pl-6 mb-1 text-[10px] text-gray-500'>
        {dedupMonth.map((m, i) => (
          <div key={i} className='w-3 sm:w-3.5 md:w-4 text-center'>{m}</div>
        ))}
      </div>
      <div className='flex gap-1 overflow-x-auto py-1'>
        ...
        <div className='flex flex-col gap-1 mr-1 text-[10px] text-gray-500'>
          {weekday.map((d, i) => (
            <div key={i} className='h-3 sm:h-3.5 md:h-4 leading-3 sm:leading-3.5 md:leading-4 flex items-center'>{i % 2 === 0 ? d : ''}</div>
          ))}
        </div>
        {/* Grid */}
        {columns.map((col, ci) => (
            ...
            {col.map(({ date, key }, ri) => {
              ...
              return (
                <div
                  ...
                  onClick={() => setTooltip(prev => ({ ...prev, text: `${key}  ${valueLabel ? valueLabel(v) : v}`, visible: true }))}
      ...
      {tooltip.visible && (
        <div className='fixed z-50 px-2 py-1 text-xs bg-black/80 text-white rounded pointer-events-none' style={{ left: tooltip.x, top: tooltip.y }}>
          {tooltip.text}
        </div>
      )}
