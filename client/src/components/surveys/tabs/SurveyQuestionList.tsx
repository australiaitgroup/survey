...
const SurveyQuestionList: React.FC<Props> = ({
      ...
      <div className='mb-4'>
        ...
        <div className='bg-purple-50 rounded-lg p-4'>
          <div className='flex items-center justify-between mb-3'>
            ...
            <div className='text-lg font-bold text-purple-600'>{s.questionCount} questions</div>
          </div>
          <div className='text-xs text-gray-500'>💡 Questions are randomized per student to ensure assessment fairness</div>
        </div>
      </div>
    );
  }
              ...
              s.multiQuestionBankConfig.map((config: any, index: number) => {
                ...
                return (
                  <div key={index} className='flex items-center justify-between p-3 bg-white rounded-lg border'>
                    <div>
                      <div className='font-medium text-gray-800'>{bank?.name || 'Unknown Bank'}</div>
                      <div className='text-sm text-gray-600'>
                    ...
                    <div className='text-lg font-bold text-blue-600'>{config.questionCount}</div>
                  </div>
                );
              })
            ) : (
              <div className='text-gray-500 text-sm text-center py-4'>No question bank configurations set</div>
            )}
            <div className='text-xs text-gray-500 mt-2'>💡 Questions are selected based on configured rules for each bank</div>
          </div>
        </div>
      </div>
    );
  ...
  if (s.sourceType === SOURCE_TYPE.MANUAL_SELECTION) {
    return (
      <div className='mb-4'>
        <h4 className='font-semibold text-gray-800 mb-3'>Manual Question Selection Survey</h4>
        <div className='bg-green-50 rounded-lg p-4'>
          <div className='flex items-center justify-between mb-3'>
            ...
            <div className='text-lg font-bold text-green-600'>{s.selectedQuestions?.length || 0} questions</div>
          </div>
          <div className='text-xs text-gray-500'>💡 Questions are pre-selected and will be the same for all students</div>
        </div>
      </div>
    );
  }
