							...
							paged.map(inv => {
								...
								return (
									<tr key={inv._id}>
										...
										<td className='px-2 py-1 border'>
											<span
												className={`px-2 py-1 rounded text-xs font-bold ${status.color === 'green' ? 'bg-green-100 text-green-700' : status.color === 'red' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700'}`}
											>
												{status.label}
											</span>
										</td>
										<td className='px-2 py-1 border space-x-2'>
											<button
												...
												onClick={() => handleCopy(inv.invitationCode)}
											>
												Copy Link
											</button>
										</td>
