                  ...
                  <span className='font-medium text-purple-600'>
                    {(() => {
                      const questionBankId = s.questionBankId && typeof s.questionBankId === 'object'
                        ...
                        : s.questionBankId;
                      return questionBanks.find(bank => bank._id === questionBankId)?.name || 'Unknown Question Bank';
                    })()}
                  </span>
                </div>
                <div className='flex justify-between'>
          ...
          <div className='bg-gray-50 rounded-lg p-4 mb-4'>
            <div className='space-y-3'>
              {s.type === SURVEY_TYPE.ASSESSMENT ? (
                <div className='flex items-center justify-between'>
                  <div>
                    <label className='block text-sm font-medium text-gray-700 mb-1'>Enhanced Assessment URL</label>
                    <div className='text-sm text-gray-600 font-mono'>{getAssessmentUrl(s.slug, companySlug)}</div>
                  </div>
                  <div className='flex flex-wrap items-center gap-1 sm:gap-2'>
                      <button className='btn-secondary btn-small' onClick={() => copyToClipboard(getAssessmentUrl(s.slug, companySlug))}>Copy URL</button>
                      <button className='btn-outline btn-small' onClick={() => window.open(getAssessmentUrl(s.slug, companySlug), '_blank')}>Open</button>
                    <button className='btn-primary btn-small' onClick={() => toggleQR(s._id)}>{showQR[s._id] ? 'Hide QR' : 'Show QR'}</button>
                  </div>
                </div>
              ) : (
                <>
                  <div className='flex items-center justify-between'>
                    <div>
                      <label className='block text-sm font-medium text-gray-700 mb-1'>Classic Survey URL</label>
                      <div className='text-sm text-gray-600 font-mono'>{getSurveyUrl(s.slug, companySlug)}</div>
                    </div>
                    <div className='flex flex-wrap items-center gap-1 sm:gap-2'>
                      <button className='btn-secondary btn-small' onClick={() => copyToClipboard(getSurveyUrl(s.slug, companySlug))}>Copy URL</button>
                      <button className='btn-outline btn-small' onClick={() => window.open(getSurveyUrl(s.slug, companySlug), '_blank')}>Open</button>
                      <button className='btn-primary btn-small' onClick={() => toggleQR(s._id)}>{showQR[s._id] ? 'Hide QR' : 'Show QR'}</button>
                    </div>
                  </div>
                </>
            ...
            {showQR[s._id] && (
              <div className='border-t border-gray-200 pt-4'>
                <QRCodeComponent url={s.type === SURVEY_TYPE.ASSESSMENT ? getAssessmentUrl(s.slug, companySlug) : getSurveyUrl(s.slug, companySlug)} />
              </div>
            )}
          </div>
