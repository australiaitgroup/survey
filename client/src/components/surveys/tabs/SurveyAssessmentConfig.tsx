...
const SurveyAssessmentConfig: React.FC<Props> = ({ survey, onEditScoring }) => {
  ...
  return (
    <>
      {(s.timeLimit || s.maxAttempts !== 1 || s.instructions || s.navigationMode !== NAVIGATION_MODE.STEP_BY_STEP) && (
            ...
            {s.navigationMode !== NAVIGATION_MODE.STEP_BY_STEP && (
              <div className='flex justify-between'>
                ...
                <span className='font-medium text-blue-600'>
                  {s.navigationMode === NAVIGATION_MODE.ONE_QUESTION_PER_PAGE ? 'One Question Per Page' : 'Step-by-step'}
                </span>
              </div>
            )}
          </div>
      ...
      {TYPES_REQUIRING_ANSWERS.includes(s.type as any) && s.scoringSettings && (
        <div className='bg-green-50 rounded-lg p-3 mb-3'>
          <div className='flex items-center justify-between mb-2'>
            ...
            <button className='text-sm text-blue-600 hover:text-blue-800' onClick={onEditScoring}>Edit Scoring Rules</button>
          </div>
          <div className='grid grid-cols-2 gap-2 text-sm'>
            <div className='flex justify-between'>
              <span className='text-gray-600'>Scoring Mode:</span>
              <span className='font-medium text-green-600'>
                {s.scoringSettings.scoringMode === SCORING_MODE.PERCENTAGE ? 'Percentage' : 'Accumulated'}
              </span>
            </div>
            <div className='flex justify-between'>
              <span className='text-gray-600'>Passing Threshold:</span>
              <span className='font-medium text-green-600'>{s.scoringSettings.passingThreshold} points</span>
            </div>
            <div className='flex justify-between'>
              <span className='text-gray-600'>Total Score:</span>
              <span className='font-medium text-green-600'>
                {s.scoringSettings.scoringMode === SCORING_MODE.PERCENTAGE ? '100' : s.scoringSettings.totalPoints} points
              </span>
            </div>
          </div>
        </div>
