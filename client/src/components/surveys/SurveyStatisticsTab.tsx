          ...
          <div className='bg-blue-50 rounded-lg p-4'>
            ...
            <div className='grid grid-cols-3 gap-4 text-sm'>
              <div className='text-center'>
                <div className='font-bold text-blue-600 text-lg'>{totals.totalResponses}</div>
                <div className='text-gray-600'>Total Responses</div>
              </div>
              <div className='text-center'>
                <div className='font-bold text-green-600 text-lg'>{totals.completionRate}%</div>
                <div className='text-gray-600'>Completion Rate</div>
              </div>
              <div className='text-center'>
                <div className='font-bold text-purple-600 text-lg'>{totals.totalQuestions}</div>
                          ...
                          <div key={opt} className='flex justify-between items-center'>
                            ...
                            <div className='flex items-center gap-2'>
                              <div className='w-20 bg-gray-200 rounded-full h-2'>
                                <div className='bg-blue-600 h-2 rounded-full' style={{ width: `${pct.toFixed(1)}%` }} />
                              </div>
                              <span className='font-medium text-blue-600 text-sm w-12'>{count as number}</span>
                              <span className='text-gray-500 text-xs w-12'>({pct.toFixed(1)}%)</span>
                            </div>
                          </div>
                        );
                      })}
                ...
                <>
                  <div className='flex justify-between items-center text-sm text-gray-600 mb-4'>
                    <div>
                      ...
                      {Math.min(responsePage * pageSize, stats.userResponses.length)}
                    </div>
                  </div>
                  {stats.userResponses
                    .slice((responsePage - 1) * pageSize, responsePage * pageSize)
                    .map(response => (
                        ...
                        <div className='grid gap-2'>
                          {Object.entries(response.answers).map(([q, a]) => (
                            <div key={q} className='flex justify-between text-sm'>
                              <div className='text-gray-700 w-1/2 pr-2 truncate'>{q}</div>
                              <div className='text-gray-900 w-1/2 pl-2 break-words'>{a}</div>
                            </div>
                          ))}
                        </div>
                      </div>
                  ...
                  {stats.userResponses.length > pageSize && (
                    <div className='flex items-center justify-between mt-2'>
                      <button
                        onClick={() => setResponsePage(prev => Math.max(1, (typeof prev === 'number' ? prev : 1) - 1))}
                      ...
                      <button
                        ...
                        disabled={responsePage >= Math.ceil(stats.userResponses.length / pageSize)}
                        className='px-3 py-1 text-sm border rounded hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed'
                      >
                        Next
                      </button>
