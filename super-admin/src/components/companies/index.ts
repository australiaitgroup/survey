...
class CompaniesComponent {
	...
	async loadCompanyDetails(companyId: string): Promise<void> {
		try {
			// Load company stats
			const statsResponse = await (window as any).API?.get(
				`/sa/companies/${companyId}/stats`
			);
			...
			// Load company users
			const usersResponse = await (window as any).API?.get(
				`/sa/companies/${companyId}/users`
			);
	...
	async suspendCompany(company: Company): Promise<void> {
		...
		try {
			const response = await (window as any).API?.put(
				`/sa/companies/${company._id}/suspend`,
				{
	...
	async activateCompany(company: Company): Promise<void> {
		try {
			const response = await (window as any).API?.put(
				`/sa/companies/${company._id}/activate`
			);
